...
mlist.scaled <- lapply(mlist.scaled, t) # It must be transposed!
...
Matrix <- list()
for(l in 1:4){
    Matrix[[l]] <- matrix(unlist(Mlist.s.d[[l]]), ncol=395, byrow=T)
}
...
save(list=ls(), file=paste0("Oyy_scaled_4patient ", K, " clusters.RData"))
...
pdf(paste0('Vary parameter 4 patient ', K, ' clusters.pdf'))
...
pdf(paste0('Centroid plot against 1st 2nd discr functions 4patient ', K, ' clusters.pdf'))
